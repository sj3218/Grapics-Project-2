...
GLuint LoadShaders(const char* vertex_file_path, const char* fragment_file_path)
{
    ...
    if (InfoLogLength > 0) {
        int length = InfoLogLength + 1;
        std::vector<char> VertexShaderErrorMessage(length);
    ...
    if (InfoLogLength > 0) {
        ...
        int length = InfoLogLength + 1;
        std::vector<char> FragmentShaderErrorMessage(length);
    ...
    if (InfoLogLength > 0) {
        ...
        int length = InfoLogLength + 1;
        std::vector<char> ProgramErrorMessage(length);
    ...
    Primitive_Enum out_primitive_type)
{
    ...
    {
    case Primitive_Enum::TriangleStrip:
        temp = "triangle_strip";
        break;

    case Primitive_Enum::LineStrip:
        temp = "line_strip";
        break;

    case Primitive_Enum::Points:
        temp = "points";
        break;
    };
    ...
    {
        int length = InfoLogLength + 1;
        std::vector<char> VertexShaderErrorMessage(static_cast<unsigned long>(length));
        glGetShaderInfoLog(VertexShaderID, InfoLogLength, nullptr, &VertexShaderErrorMessage[0]);
        printf("%s\n", &VertexShaderErrorMessage[0]);
    }
    ...
    {
        int length = InfoLogLength + 1;
        std::vector<char> GeometryShaderErrorMessage((unsigned long)(length));
    ...
    {
        int length = InfoLogLength + 1;
        std::vector<char> FragmentShaderErrorMessage(static_cast<unsigned long>(length));
        glGetShaderInfoLog(FragmentShaderID, InfoLogLength, nullptr, &FragmentShaderErrorMessage[0]);
        printf("%s\n", &FragmentShaderErrorMessage[0]);
    }
    ...
    {
        int length = InfoLogLength + 1;
        std::vector<char> ProgramErrorMessage(static_cast<unsigned long>(length));
        glGetProgramInfoLog(ProgramID, InfoLogLength, nullptr, &ProgramErrorMessage[0]);
        printf("%s\n", &ProgramErrorMessage[0]);
    }
