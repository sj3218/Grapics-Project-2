...
//    clock_t  startTime, endTime;
    ...
    int count = _currentMesh->getVertexBufferSize();
    glm::vec3 translate = _currentMesh->getModelCentroid();
    glm::vec3 scale = glm::vec3(_currentMesh->getModelScale());
    scale = glm::vec3(2.f) / scale;

    for (int i = 0; i < count; ++i)
    {
        glm::vec3 point = _currentMesh->vertexBuffer.at(i);
        _currentMesh->vertexBuffer.at(i) = glm::vec3(glm::scale(scale)*glm::translate(-translate) * glm::vec4(point,1.f));
    }
    ...
    _currentMesh->SetFaceBuffer(bFlipNormals);
    return timeDuration;
}
...
int OBJReader::ReadOBJFile_BlockIO( std::string filepath )
{
    ...
    else if (count > 0)
    {
        ...
        fileContents = (char *)malloc(sizeof(char) * (countPlusOne));
        ...
        while( token != nullptr )
        {
            ...
            char ObjLine[256];
            strncpy_s( ObjLine, 256, currPtr, numChars );
            ObjLine[numChars]='\0';
